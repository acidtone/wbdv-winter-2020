...
MongoClient.connect(uri,{ useUnifiedTopology: true,useNewUrlParser: true },  function(err, client) {
  ...
  // Closing the connection causes an Error. This is often problematic in asynchronous environments because `defCol.insertMany` is still pending. It will arrive back to the server after the connection has been closed.
  client.close();
});
